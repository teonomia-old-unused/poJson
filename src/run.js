...
async function  traslate (it) {
  ...
  const poBuff = await F.rf(process.cwd()+'/src/data.test/gettext_po_sample_file.po'); const po = await poBuff.toString()
  const pojson = r.po2poJson(po)
  F.wf(process.cwd()+'/src/dataProcessed/gettext_po_sample_file.po.json', JSON.stringify(pojson))
